...
const ResponseDialog = ({
	...
	const questions = useMemo(() => {
		...
		// Use a Map to store unique questions by questionId
		...
		responses[0].answers?.map((answer) => {
			if (!questionMap.has(answer.questionId)) {
				questionMap.set(answer.questionId, {
					...
					type: determineQuestionType(answer.answer), // Helper function to determine type
				});
			}
		});

		return Array.from(questionMap.values()); // Convert Map values to an array
	}, [responses]);
